...
double gripper_2D_t::distance(double* point1,double* point2)
{
	return std::sqrt( (point1[0]-point2[0]) * (point1[0]-point2[0]) + (point1[1]-point2[1]) * (point1[1]-point2[1]));
}

void gripper_2D_t::random_particles(double* destination, double* state, double radius)
{
	destination[0] = state[0] + uniform_random(-radius, radius);
	destination[1] = state[1] + uniform_random(-radius, radius);
}

void gripper_2D_t::random_state(double* state)
{
	state[0] = uniform_random(MIN_X,MAX_X);
	state[1] = uniform_random(MIN_Y,MAX_Y);
}

void gripper_2D_t::random_control(double* control)
{
	control[0] = uniform_random(MIN_SPEED, MAX_SPEED);
	control[1] = uniform_random(MIN_SPEED, MAX_SPEED);
}

bool gripper_2D_t::propagate( double* start_state, double* control, int min_step, int max_step, double* result_state, double& duration )
{	
...
bool gripper_2D_t::convergent_propagate( const bool &random_time, double* start_state,std::vector<double*> &start_particles, double* control, int min_step, int max_step, double* result_state, std::vector<double*> &result_particles, double& duration, double& cost )
{	
	double local_cost = 0;
	temp_state[0] = start_state[0]; temp_state[1] = start_state[1];
...
void gripper_2D_t::enforce_bounds(double* state)
{
	if(state[0]<MIN_X)
		state[0]=MIN_X;
...
bool gripper_2D_t::check_collision(double* state)
{
...
int gripper_2D_t::location_in_obstacle(double* state, int obstacles_id, double* corners, double* curv_cen)
{
	int pos_record = 0;
...
int gripper_2D_t::enforce_obstacle_bounds(double* state, int obstacles_id, double* corners, double* curv_cen, double lower_angle_bound, double upper_angle_bound)
{
	int enforce_pos = -1;
...
svg::Point gripper_2D_t::visualize_point(double* state, svg::Dimensions dims)
{
...
std::string gripper_2D_t::export_point(double* state)
{
	std::stringstream s;
